...
- Landsmanを読んだ。
  ...
  - 2.1を読み終わった。なんとなく、density operatorが状態、random variableがオブザーバブルのように見える。また$p_a$は$a$のスペクトル上の確率分布を与える、つまり$a$のスペクトル分解を与えると思ったので良いと思う。
  - 2.2節の頭（$B(H)$上の状態の定義、 $B(H)\_{\mathrm{sa}}$上の状態の定義）を読んだ。$B(H)$上の状態が、物理屋にとっての状態の定義と異なり、$B(H)\_{\mathrm{sa}}$上の状態が物理屋にとっての状態になるらしい。どういうこと
